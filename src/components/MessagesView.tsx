...
import { Search, Edit, Image, X, MessageCircle, ArrowLeft, Video, Palette, Send } from "lucide-react";
...
const MessagesView = () => {
  ...
  // Simulate other person typing (in real app, this would come from websocket)
  useEffect(() => {
    if (selectedChat) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // In real app, emit typing event to other user via websocket
                ...
                <div className="flex items-center gap-3 w-full">
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold text-foreground text-sm truncate">{conv.user}</span>
        ...
        <div className="relative z-10 flex-1 flex flex-col">
          {selectedChat ? (
            <>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      ...
                      {textColorOptions.map((color) => (
                        <DropdownMenuItem key={color.id} onClick={() => setTextColor(color.class)}>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56 max-h-96 overflow-y-auto">
                      ...
                      {backgroundOptions.map((bg) => (
                        <DropdownMenuItem key={bg.id} onClick={() => handleBackgroundSelect(bg.gradient || '')}>
                          <div className="flex items-center gap-2 w-full">
                            <div className={`w-8 h-8 rounded ${bg.color || ''}`} style={bg.gradient ? { background: bg.gradient } : {}} />
                            <span>{bg.name}</span>
                          </div>
                        </DropdownMenuItem>
                      ))}
                ...
                <ScrollArea className={`${videoCallActive ? 'w-1/2' : 'w-full'} px-3 md:px-4 transition-all duration-300`}>
                  <div className="py-4 space-y-4">
                    <div className="flex justify-center">
                      <span className="text-xs text-muted-foreground bg-card/80 px-3 py-1 rounded-full">Today</span>
                    </div>
                    {/* Sample messages */}
                    ...
                    {isTyping && (
                      <div className="flex gap-2">
                        ...
                        <div className="bg-card/90 backdrop-blur-sm px-4 py-2 rounded-2xl rounded-tl-sm">
                    ...
                    <div className="flex-1 flex items-center justify-center relative">
                      ...
                      <div className="relative z-10 text-center">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Video call with {conversations.find((c) => c.id === selectedChat)?.user}
                        </p>
              ...
              {/* Message Input */}
              <div className="h-16 px-3 md:px-4 flex items-center gap-2 border-t border-border bg-card/90 backdrop-blur-sm">
                <Input
                  placeholder="Type a message..."
                  value={message}
                  onChange={handleInputChange}
                  className={`flex-1 bg-accent ${textColor}`}
                />
